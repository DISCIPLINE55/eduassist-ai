...
export default function DocumentViewerScreen() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const text = formatDocumentAsText(doc.title, doc.type, doc.content);
      // Convert plain text to styled HTML for printing
      const lines = text.split("\n");
      const htmlLines = lines.map((line) => {
        if (line.startsWith("===")) return `<hr style="border:1px solid #e5e7eb;margin:10px 0"/>`;
        if (line.startsWith("## ")) return `<h2 style="font-size:16px;font-weight:700;color:#052e16;margin:16px 0 6px">${line.replace("## ", "")}</h2>`;
        if (line.startsWith("# ")) return `<h1 style="font-size:22px;font-weight:900;color:#052e16;margin:0 0 8px">${line.replace("# ", "")}</h1>`;
        if (line.startsWith("  • ")) return `<p style="margin:3px 0 3px 16px;font-size:13px;color:#374151">• ${line.slice(4)}</p>`;
        if (line.startsWith("• ")) return `<p style="margin:4px 0 4px 8px;font-size:13px;color:#374151">• ${line.slice(2)}</p>`;
        if (line.trim() === "") return `<br/>`;
        return `<p style="margin:4px 0;font-size:13px;color:#374151;line-height:1.6">${line}</p>`;
      });
      const html = `
        <!DOCTYPE html><html><head><meta charset="utf-8"/>
        <style>
          body { font-family: -apple-system, 'Helvetica Neue', Arial, sans-serif; padding: 32px 40px; color: #1f2937; }
          @media print { body { padding: 20px; } }
        </style></head>
        <body>
          <div style="border-top:4px solid #052e16;padding-top:16px;margin-bottom:24px">
            <p style="font-size:11px;color:#6b7280;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin:0">EduAssist AI — ${DOCUMENT_TYPE_LABELS[doc.type]}</p>
          </div>
          ${htmlLines.join("")}
          <div style="margin-top:32px;border-top:1px solid #e5e7eb;padding-top:12px">
            <p style="font-size:10px;color:#9ca3af;text-align:center">Generated by EduAssist AI · Ghana NaCCA Aligned</p>
          </div>
        </body></html>`;
      ...
      if (process.env.EXPO_OS === "web") {
        ...
        showMsg("Print dialog opened");
      } else {
        const { uri } = await Print.printToFileAsync({ html, base64: false });
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(uri, { mimeType: "application/pdf", dialogTitle: `Share ${doc.title}` });
        } else {
          showMsg("PDF saved — check your Files app");
        }
      }
    } catch {
      showErr("PDF export failed. Please try again.");
    } finally {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      {/* Presentation "Present" button — only for presentation type */}
      {doc.type === "presentation" ? (
        <View className="px-4 pt-3 pb-1">
          <Pressable
            className="flex-row items-center justify-center gap-2 bg-primary rounded-2xl py-3.5 active:opacity-80"
            style={{ borderCurve: "continuous" }}
            onPress={() => setShowPresentation(true)}
          >
            <Presentation size={18} color="#fff" />
            <Text className="text-primary-foreground font-bold text-base">Present Fullscreen</Text>
          </Pressable>
        </View>
      ) : null}
